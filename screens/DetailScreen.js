...
const DetailScreen = () =>{
    ...
    const sendImg = async () => {
        ...
        fd.append('file', data);
        axios.post(`http://1.176.185.164:5000/predict`, fd, {
            headers: {
    ...
    const saveResult = async () => {
        ...
        await axios.post(`http://1.176.185.164:5000/save?id=${userID}`,JSON.stringify({
            image_data: foodImg,
            foodData: foodData
        }),
        {
        ...
        if(visible){
            ...
            result.push(
                <View style={styles.textBox} key={i}>
                    <Text style={{fontSize:15,marginLeft:'5%',color:'#F3F2F4'}}>음식이름 : </Text>
                    <SelectDropdown
                        data={foodList}
                        ...
                        renderDropdownIcon={isOpened => {
                            return <Ionicons name={isOpened ? 'chevron-up' : 'chevron-down'} color={'#f3f2f4'} size={12} />;
                        }}
                        ...
                        searchInputTxtColor={'#F3F2F4'}
                        searchPlaceHolder={'Search here'}
                        searchPlaceHolderColor={'#F8F8F8'}
                        renderSearchInputLeftIcon={() => {
                            return <Ionicons name={'search'} color={'#F3F2F4'} size={16} />;
                        }}
                    />
                    <Text style={{fontSize:15,marginLeft:'2%',color:'#F3F2F4'}}>양 : </Text>
                    <SelectDropdown
    ...
    return(
        <View style={styles.screen}>
            ...
            <View style={styles.twobutton}>
                    <TouchableOpacity onPress={plusfood}><Ionicons name={'add'} color={'#F2F3F4'} size={24} /></TouchableOpacity>
                    <TouchableOpacity onPress={deletefood}><Ionicons name={'ios-remove-outline'} color={'#F2F3F4'} size={24} /></TouchableOpacity>
            </View>
...
const styles=StyleSheet.create({
    ...
    dropdown2BtnStyle: {
      ...
      dropdown2RowStyle: {backgroundColor: '#20212D',height:30, borderBottomColor: 'grey',color:'white'},
      dropdown2RowTxtStyle: {
        color: '#F3F2F4',
        textAlign: 'center',
        // fontWeight: 'bold',
      },
      dropdown2SelectedRowStyle: {backgroundColor: 'rgba(255,255,255,0.2)',color:'#F2F3F4'},
      dropdown2searchInputStyleStyle: {
        color:'F3F2F4',
        backgroundColor: '#20212D',
        borderBottomWidth: 1,
        borderBottomColor: '#20212D',
      },
      dropdown3BtnStyle: {
        marginLeft:'3%',
        width: '22%',
